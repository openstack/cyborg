...
if [ -n "$DEV" ]; then
    ...
    echo BASE=$BASE
    UART_OFFSET=0
    [ "$(cat $DEV/vendor)" = "0x10a9" ] && \
        [ "$(cat $DEV/device)" = "0x0003" ] && UART_OFFSET=0x20178
...
if [ -n "$DEV" ] && command -v devmem >/dev/null 2>&1; then
    ...
    for t in /dev/ttyS32 /dev/ttyS33 /dev/ttyS34 /dev/ttyS35 \
        /dev/ttyS1 /dev/ttyS2 /dev/ttyS3 /dev/ttyS4; do
        [ -e $t ] || continue
        echo TRY_TTY=$t
        if (
            exec 7<>$t || exit 1
            stty -F $t raw -echo -icanon clocal -hupcl \
                min 0 time 20 9600 2>&1 || exit 1
            printf "%s" "$TEST_STR" >&7
