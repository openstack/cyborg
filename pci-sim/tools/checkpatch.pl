...
Options:
  ...
  -g, --git                  treat FILE as a single comit or git revision range
                             single git comit with:
                               <rev>
                               <rev>^
                               <rev>~n
                             multiple git comits with:
                               <rev1>..<rev2>
                               <rev1>...<rev2>
                               <rev>-<count>
  ...
  --ignore-perl-version      override checking of perl version.  expect
                             runtime errors.
  --no-spelling             Disable spelling/typo checks; spelling is handled
                             by the repository codespell pre-comit hook
...
my $DO_WHILE_0_ADVICE = q{
   ...
   There's also macros called inside an if (block), which "return" an
...
sub seed_camelcase_includes {
	...
	if (-e "$gitroot") {
		my $git_last_include_comit = `${git_command} log --no-merges --pretty=format:"%h%n" -1 -- include`;
		chomp $git_last_include_comit;
		$camelcase_cache = ".checkpatch-camelcase.git.$git_last_include_comit";
	} else {
		my $last_mod_date = 0;
...
sub git_comit_info {
	...
	my $output = `${git_command} log --no-color --format='%H %s' -1 $comit 2>&1`;
	$output =~ s/^\s*//gm;
	my @lines = split("\n", $output);

	return ($id, $desc) if ($#lines < 0);

	if ($lines[0] =~ /^error: short SHA1 $comit is ambiguous/) {
# Maybe one day convert this block of bash into something that returns
# all matching comit ids, but it's very slow...
#
#		echo "checking comits $1..."
#		git rev-list --remotes | grep -i "^$1" |
#		while read line ; do
#		    git log --format='%H %s' -1 $line |
#		    echo "comit $(cut -c 1-12,41-)"
#		done
	} elsif ($lines[0] =~ /^fatal: ambiguous argument '$comit': unknown revision or path not in the working tree\./ ||
		 $lines[0] =~ /^fatal: bad object $comit/) {
		$id = undef;
	} else {
...
# If input is git comits, extract all comits from the comit expressions.
# For example, HEAD-3 means we need check 'HEAD, HEAD~1, HEAD~2'.
die "$P: No git repository found\n" if ($git && !-e "$gitroot");

if ($git) {
	my @comits = ();
	foreach my $comit_expr (@ARGV) {
		my $git_range;
		if ($comit_expr =~ m/^(.*)-(\d+)$/) {
			$git_range = "-$2 $1";
		} elsif ($comit_expr =~ m/\.\./) {
			$git_range = "$comit_expr";
		} else {
			$git_range = "-1 $comit_expr";
		}
		...
		foreach my $line (split(/\n/, $lines)) {
			...
			my $subject = $2;
			unshift(@comits, $sha1);
			$git_comits{$sha1} = $subject;
		}
	}
	die "$P: no git comits after extraction!\n" if (@comits == 0);
	@ARGV = @comits;
}
...
for my $filename (@ARGV) {
	...
	if ($filename eq '-') {
		...
		$vname = "Comit " . substr($filename, 0, 12) . ' ("' . $git_comits{$filename} . '")';
	} else {
		$vname = $filename;
	}
	while (<$FILE>) {
...
sub process {
	...
	my $in_comit_log = 0;		#Scanning lines before patch
	my $has_patch_separator = 0;	#Found a --- line
	my $has_comit_log = 0;		#Encountered lines before patch
	my $comit_log_lines = 0;	#Number of comit log lines
	my $comit_log_possible_stack_dump = 0;
	my $comit_log_long_line = 0;
	my $comit_log_has_diff = 0;
	my $reported_maintainer_file = 0;
	my $non_utf8_charset = 0;

	my $last_git_comit_id_linenr = -1;
...
# Verify the existence of a comit log if appropriate
# 2 is used because a $signature is counted in $comit_log_lines
		if ($in_comit_log) {
			if ($line !~ /^\s*$/) {
				$comit_log_lines++;	#could be a $signature
			}
		} elsif ($has_comit_log && $comit_log_lines < 2) {
			WARN("COMMIT_MESSAGE",
			     "Missing comit description - Add an appropriate one\n");
			$comit_log_lines = 2;	#warn only once
		}

# Check if the comit log has what seems like a diff which can confuse patch
		if ($in_comit_log && !$comit_log_has_diff &&
		    (($line =~ m@^\s+diff\b.*a/([\w/]+)@ &&
		      $line =~ m@^\s+diff\b.*a/[\w/]+\s+b/$1\b@) ||
		     $line =~ m@^\s*(?:\-\-\-\s+a/|\+\+\+\s+b/)@ ||
		     $line =~ m/^\s*\@\@ \-\d+,\d+ \+\d+,\d+ \@\@/)) {
			ERROR("DIFF_IN_COMMIT_MSG",
			      "Avoid using diff content in the comit message - patch(1) might not work\n" . $herecurr);
			$comit_log_has_diff = 1;
		}
...
# Check Fixes: styles is correct
		if (!$in_header_lines &&
		    $line =~ /^\s*(fixes:?)\s*(?:comit\s*)?([0-9a-f]{5,40})(?:\s*($balanced_parens))?/i) {
			my $tag = $1;
			my $orig_comit = $2;
			my $title;
			...
			my $id_length = not ($orig_comit =~ /^[0-9a-f]{12,40}$/i);
			my $id_case = not ($orig_comit !~ /[A-F]/);

			my $id = "0123456789ab";
			my ($cid, $ctitle) = git_comit_info($orig_comit, $id,
							     $title);
...
# Check if the comit log is in a possible stack dump
		if ($in_comit_log && !$comit_log_possible_stack_dump &&
		    ($line =~ /^\s*(?:WARNING:|BUG:)/ ||
		     $line =~ /^\s*\[\s*\d+\.\d{6,6}\s*\]/ ||
					# timestamp
		     $line =~ /^\s*\[\<[0-9a-fA-F]{8,}\>\]/) ||
			...
			$comit_log_possible_stack_dump = 1;
		}

# Check for line lengths > 75 in comit log, warn once
		if ($in_comit_log && !$comit_log_long_line &&
		    length($line) > 75 &&
		    !($line =~ /^\s*[a-zA-Z0-9_\/\.]+\s+\|\s+\d+/ ||
		      ...
		      $comit_log_possible_stack_dump)) {
			WARN("COMMIT_LOG_LONG_LINE",
			     "Prefer a maximum 75 chars per line (possible unwrapped comit description?)\n" . $herecurr);
			$comit_log_long_line = 1;
		}

# Reset possible stack dump if a blank line is found
		if ($in_comit_log && $comit_log_possible_stack_dump &&
		    $line =~ /^\s*$/) {
			$comit_log_possible_stack_dump = 0;
		}
...
# Check for lines starting with a #
		if ($in_comit_log && $line =~ /^#/) {
			if (WARN("COMMIT_COMMENT_SYMBOL",
				 "Comit log lines starting with '#' are dropped by git as comments\n" . $herecurr) &&
			    $fix) {
				$fixed[$fixlinenr] =~ s/^/ /;
			}
		}
...
# Check for auto-generated unhandled placeholder text (mostly for cover letters)
		if (($in_comit_log || $in_header_lines) &&
		    $rawline =~ /(?:SUBJECT|BLURB) HERE/) {
			ERROR("PLACEHOLDER_USE",
			      "Placeholder text detected\n" . $herecurr);
		}

# Check for git id comit length and improperly formed comit descriptions
# A correctly formed comit description is:
#    comit <SHA-1 hash length 12+ chars> ("Complete comit subject")
# with the comit subject '("' prefix and '")' suffix
# This is a fairly complicated block as it tests for what appears to be
# bare SHA-1 hash with  minimum length of 5.  It also avoids several types of
# possible SHA-1 matches.
# A comit match can span multiple lines so this block attempts to find a
# complete typical comit on a maximum of 3 lines
		if ($perl_version_ok &&
		    $in_comit_log && !$comit_log_possible_stack_dump &&
		    $line !~ /^\s*(?:Link|Patchwork|http|https|BugLink|base-comit):/i &&
		    $line !~ /^This reverts comit [0-9a-f]{7,40}/ &&
		    (($line =~ /\bcomit\s+[0-9a-f]{5,}\b/i ||
		      ($line =~ /\bcomit\s*$/i && defined($rawlines[$linenr]) && $rawlines[$linenr] =~ /^\s*[0-9a-f]{5,}\b/i)) ||
			...
			my $space = 1;
			my $id = '0123456789ab';
			my $orig_desc = "comit description";
			my $description = "";
			my $herectx = $herecurr;
			my $has_parens = 0;
			my $has_quotes = 0;
			...
			my $input = $line;
			if ($line =~ /(?:\bcomit\s+[0-9a-f]{5,}|\bcomit\s*$)/i) {
				for (my $n = 0; $n < 2; $n++) {
					if ($input =~ /\bcomit\s+[0-9a-f]{5,}\s*($balanced_parens)/i) {
						$orig_desc = $1;
						$has_parens = 1;
						# Always strip leading/trailing parens then double quotes if existing
			...
			if ($input =~ /\b(c)omit\s+([0-9a-f]{5,})\b/i) {
				$init_char = $1;
				$orig_comit = lc($2);
				$short = 0 if ($input =~ /\bcomit\s+[0-9a-f]{12,40}/i);
				$long = 1 if ($input =~ /\bcomit\s+[0-9a-f]{41,}/i);
				$space = 0 if ($input =~ /\bcomit [0-9a-f]/i);
				$case = 0 if ($input =~ /\b[Cc]omit\s+[0-9a-f]{5,40}[^A-F]/);
			} elsif ($input =~ /\b([0-9a-f]{12,40})\b/i) {
				$orig_comit = lc($1);
			}

			($id, $description) = git_comit_info($orig_comit,
							      $id, $orig_desc);

			if (defined($id) &&
			    ($short || $long || $space || $case || ($orig_desc ne $description) || !$has_quotes) &&
			    $last_git_comit_id_linenr != $linenr - 1) {
				ERROR("GIT_COMMIT_ID",
				      "Please use git comit description style 'comit <12+ chars of sha1> (\"<title line>\")' - ie: '${init_char}omit $id (\"$description\")'\n" . $herectx);
			}
			#don't report the next line if this line ends in comit and the sha1 hash is the next line
			$last_git_comit_id_linenr = $linenr if ($line =~ /\bcomit\s*$/i);
		}
...
# Check for added, moved or deleted files
		if (!$reported_maintainer_file && !$in_comit_log &&
...
# UTF-8 regex found at http://www.w3.org/International/questions/qa-forms-utf-8.en.php
		if (($realfile =~ /^$/ || $line =~ /^\+/) &&
			...
			CHK("INVALID_UTF8",
			    "Invalid UTF-8, patch and comit message should be encoded in UTF-8\n" . $hereptr);
		}

# Check if it's the start of a comit log
# (not a header line and we haven't seen the patch filename)
		if ($in_header_lines && $realfile =~ /^$/ &&
		    !($rawline =~ /^\s+(?:\S|$)/ ||
		      $rawline =~ /^(?:comit\b|from\b|[\w-]+:)/i)) {
			$in_header_lines = 0;
			$in_comit_log = 1;
			$has_comit_log = 1;
		}

# Check if there is UTF-8 in a comit log when a mail header has explicitly
# declined it, i.e defined some charset where it is missing.
		...
		if ($in_comit_log && $non_utf8_charset && $realfile =~ /^$/ &&
		    $rawline =~ /$NON_ASCII_UTF8/) {
			WARN("UTF8_BEFORE_PATCH",
			    "8-bit UTF-8 used in possible comit log\n" . $herecurr);
		}

# Check for absolute kernel paths in comit message
		if ($tree && $in_comit_log) {
			while ($line =~ m{(?:^|\s)(/\S*)}g) {
				my $file = $1;
...
# check for invalid comit id
		if ($in_comit_log && $line =~ /(^fixes:|\bcomit)\s+([0-9a-f]{6,40})\b/i) {
			my $id;
			my $description;
			($id, $description) = git_comit_info($2, undef, undef);
			if (!defined($id)) {
				WARN("UNKNOWN_COMMIT_ID",
				     "Unknown comit id '$2', maybe rebased or not pulled?\n" . $herecurr);
			}
		}

# check for repeated words separated by a single space
# avoid false positive from list command eg, '-rw-r--r-- 1 root root'
		if (($rawline =~ /^\+/ || $in_comit_log) &&
		    $rawline !~ /[bcCdDlMnpPs\?-][rwxsStT-]{9}/) {
			pos($rawline) = 1 if (!$in_comit_log);
			while ($rawline =~ /\b($word_pattern) (?=($word_pattern))/g) {
				...
				my $start_char = '';
				my $end_char = '';
				$start_char = substr($rawline, $start_pos - 1, 1) if ($start_pos > ($in_comit_log ? 0 : 1));
...
# statements should be at the same indent.
		if ($line=~/\bswitch\s*\(.*\)/) {
			...
			shift(@ctx);
			for my $ctx (@ctx) {
				my ($clean, $cindent) = line_stats($ctx);
...
# check for static initialisers.
		if ($line =~ /^\+.*\bstatic\s.*=\s*($zero_initializer)\s*;/) {
			if (ERROR("INITIALISED_STATIC",
				  "do not initialise statistics to $1\n" .
				      $herecurr) &&
			    $fix) {
		...
		if ($line =~ /\bMODULE_LICENSE\s*\(\s*($String)\s*\)/) {
			...
			if (!$file && $extracted_string eq '"GPL v2"') {
				if (WARN("MODULE_LICENSE",
				     "Prefer \"GPL\" over \"GPL v2\" - see comit bf7fbeeae6db (\"module: Cure the MODULE_LICENSE \"GPL\" vs. \"GPL v2\" bogosity\")\n" . $herecurr) &&
				    $fix) {
...
# check for uninitialized pointers with __free attribute
	...
	if ($is_patch && $has_comit_log && $chk_fixes_tag) {
		if ($needs_fixes_tag ne "" && !$is_revert && !$fixes_tag) {
			WARN("MISSING_FIXES_TAG",
				 "The comit message has '$needs_fixes_tag', perhaps it also needs a 'Fixes:' tag?\n");
		}
	}
	if ($is_patch && $has_comit_log && $chk_signoff) {
		if ($signoff == 0) {
