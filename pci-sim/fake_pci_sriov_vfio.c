...
static int pci_sim_vfio_open_device(struct vfio_device *core_vdev)
{
	struct pci_sim_vfio_vf *sim =
		container_of(core_vdev, struct pci_sim_vfio_vf, core.vdev);
	int ret;
		...
		if (iswrite) {
			...
			if (vfio_uart_trace)
				pr_info("fake_pci: vfio uart W off=0x%llx reg=%u val=0x%02x\n",
					(unsigned long long)(pos + done), reg,
					val);
...
static int pci_sim_vfio_copy_config_value(char __user *buf, loff_t pos,
					...
					struct vfio_info_cap *caps)
{
	...
	info->size = vfio_guest_8250_compat ? PCI_SIM_VFIO_BAR0_SIZE :
					      BAR0_SIZE;
...
{
	unsigned int index = vma->vm_pgoff >>
