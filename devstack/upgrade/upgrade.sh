...
[[ -d "$SAVE_DIR/etc.cyborg" ]] || \
    cp -pr "$CYBORG_CONF_DIR" "$SAVE_DIR/etc.cyborg"

# Install the target upgrade
install_cyborg

# calls upgrade-cyborg for specific release
upgrade_project cyborg "$RUN_DIR" \
    "$BASE_DEVSTACK_BRANCH" "$TARGET_DEVSTACK_BRANCH"
...
if [ "$ret_val" -gt 1 ] ; then
    ...
    echo "WARNING: Status check failed, we're going to attempt to" \
