...
class TestExtARQObject(base.DbTestCase):
    ...
    def test_get(self, mock_from_db_obj):
        ...
        with mock.patch.object(
            self.dbapi, 'extarq_get', autospec=True
        ) as mock_extarq_get:
    ...
    def test_list(self, mock_from_db_obj):
        ...
        mock_from_db_obj.return_value = self.fake_obj_extarqs[0]
        with mock.patch.object(
            self.dbapi, 'extarq_list', autospec=True
        ) as mock_get_list:
    ...
    def test_create(self, mock_from_db_obj):
        ...
        mock_from_db_obj.return_value = self.fake_obj_extarqs[0]
        with mock.patch.object(
            self.dbapi, 'extarq_create', autospec=True
        ) as mock_extarq_create:
    ...
    def test_apply_patch_to_bad_arq_state(
        ...
        for state in set(constants.ARQ_STATES) - set(good_states):
            ...
            self.assertRaises(
                exception.ARQBadState,
                objects.ExtARQ.apply_patch,
                self.context,
                patch_list,
                valid_fields,
            )
    ...
    @mock.patch(
        'cyborg.objects.extarq.ext_arq_job.ExtARQJobMixin.'
    ...
    def test_destroy(self, mock_from_db_obj, mock_obj_extarq):
        ...
        with mock.patch.object(
            ...
            mock_extarq_get.return_value = db_extarq
            with mock.patch.object(
                self.dbapi, 'extarq_delete', autospec=True
            ) as mock_extarq_delete:
    ...
    def test_allocate_attach_handle(self, mock_check_state):
        ...
        self.assertRaises(
            ...
            obj_extarq._allocate_attach_handle,
            self.context,
            fake_dep,
        )
    ...
    def test_deallocate_attach_handle_with_error_log(
        ...
        msg = "Failed to deallocate attach handle %s for ARQ %s.Reason: %s"
    ...
    def test_save(self, mock_from_db_obj, mock_obj_extarq):
        ...
        mock_obj_extarq.return_value = self.fake_obj_extarqs[0]
        with mock.patch.object(
            self.dbapi, 'extarq_update', autospec=True
        ) as mock_extarq_update:
            obj_extarq = objects.ExtARQ.get(self.context, uuid)
    ...
    def test_get_arq_bind_statuses(self):
        ...
        bind_status = constants.ARQ_BIND_STATES_STATUS_MAP[constants.ARQ_BOUND]
    ...
    def _test_get_arq_bind_statuses_exception(self):
        ...
        self.assertRaises(
            exception.ARQBadState,
            objects.ExtARQ.get_arq_bind_statuses,
            arq_list,
        )
    ...
    @mock.patch('cyborg.db.sqlalchemy.api.Connection.attach_handle_get_by_id')
    @mock.patch('cyborg.db.sqlalchemy.api.Connection.device_profile_get_by_id')
    def test_fill_obj_extarq_fields(
        self, mock_get_devprof, mock_get_ah, mock_get_obj_dep, mock_obj_devprof
    ):
        ...
        self.assertEqual(
            out_db_extarq['device_profile_name'], db_devprof['name']
        )
        ...
        self.assertEqual(
            out_db_extarq['device_profile_group'],
    ...
    @mock.patch('cyborg.db.sqlalchemy.api.Connection.attach_handle_get_by_id')
    @mock.patch('cyborg.db.sqlalchemy.api.Connection.device_profile_get_by_id')
    def test_fill_obj_extarq_fields_with_dep_id(
        self, mock_get_devprof, mock_get_ah, mock_get_obj_dep, mock_obj_devprof
    ):
        in_db_extarq = self.fake_db_extarqs[0]
        # Since state is not 'Bound', attach_handle_get_by_id is not called.
        ...
        mock_get_devprof.return_value = db_devprof
        mock_obj_devprof.return_value = obj_devprof
        mock_get_obj_dep.return_value = fake_deployable.fake_deployable_obj(
            self.context, uuid=self.deployable_uuids[0]
        )
