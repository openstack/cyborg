...
class TestFPGAExtARQObject(base.DbTestCase):
    def setUp(self):
        ...
        self.class_fgpa_dbs = dict(zip(classes, self.fake_fpga_db_extarqs))
    ...
    def test_get_bitstream_id_return_None(self, mock_from_db_obj):
        ...
        with mock.patch.object(self.dbapi, 'extarq_get') as mock_extarq_get:
    ...
    def test_get_bitstream_md(self, mock_check_state, mock_conn):
        ...
        md = obj_extarq.get_bitstream_md(self.context, fake_dep, fun_id, bs_id)
        self.assertDictEqual(self.images_md["/images"][0], md)
    ...
    def test_need_extra_bind_job(self, mock_check_state, mock_conn):
        ...
        self.assertRaises(
            exception.InvalidParameterValue,
            obj_extarq._need_extra_bind_job,
            self.context,
            fake_dep,
        )
    ...
    def test_do_programming(
        self, mock_cpid_list, mock_check_state, mock_conn, mock_program
    ):
        dep_uuid = self.deployable_uuids[0]
        fake_dep = fake_deployable.fake_deployable_obj(
            self.context, uuid=dep_uuid
    ...
    @mock.patch('cyborg.objects.ExtARQ.update_check_state')
    @mock.patch('cyborg.objects.Deployable.get_cpid_list')
    def test_do_programming_with_not_one_cp(
        self, mock_cpid_list, mock_check_state
    ):
        dep_uuid = self.deployable_uuids[0]
        fake_dep = fake_deployable.fake_deployable_obj(
            self.context, uuid=dep_uuid
    ...
    @mock.patch('cyborg.common.placement_client.PlacementClient.__init__')
    @mock.patch(
        'cyborg.common.placement_client.PlacementClient.add_traits_to_rp'
    )
    @mock.patch(
