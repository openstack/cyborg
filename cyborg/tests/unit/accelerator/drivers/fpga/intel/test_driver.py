...
class TestIntelFPGADriver(base.TestCase):
    ...
    def test_discover(self):
        ...
        for i in range(len(fpgas)):
            ...
            self.assertEqual(
                expected[i]['controlpath_id'], fpga_dict['controlpath_id']
            )
            self.assertEqual(
            ...
            self.assertEqual(
                attach_handle_list[i][0], fpga_attach_handle_list[0].as_dict()
            )

    @mock.patch(
        'cyborg.accelerator.drivers.fpga.intel.driver._fpga_program_privileged'
    )
