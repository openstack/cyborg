...
class FPGAExtARQ(ExtARQ):
    ...
    def _get_bitstream_md_from_bitstream_id(self, bitstream_id):
        ...
        if resp:
            ...
            LOG.warning('Failed to get image for bitstream (%s)', bitstream_id)
            return None
        ...
        if resp:
            ...
            if not isinstance(image_list, list):
                raise exception.InvalidType(
                    obj='image', type=type(image_list), expected='list'
                )
            if len(image_list) != 1:
                raise exception.ExpectedOneObject(
                    obj='image', count=len(image_list)
                )
            LOG.info(
                '[arqs:objs] For function id (%s), got bitstream id (%s)',
                function_id,
                image_list[0]['id'],
            )
            return image_list[0]
        else:
            LOG.warning('Failed to get image for function (%s)', function_id)
            return None
    ...
    def get_bitstream_md(self, context, deployable, function_id, bitstream_id):
        ...
        LOG.info(
            "Get bitstream metadata for deployable(uuid:%s).", deployable.uuid
        )
        ...
        bitstream_md = (
            ...
            if bitstream_id
            else self._get_bitstream_md_from_function_id(function_id)
        )
        if bitstream_md:
            LOG.info(
                'ARQ %s get bitstream metadata:%s from image registry.',
                self.arq.uuid,
                bitstream_md,
            )
        else:
            self.update_check_state(context, constants.ARQ_BIND_FAILED)
            LOG.error(
                'Can not get bitstream metadata from image registry '
                'for ARQ %s',
                self.arq.uuid,
            )
        return bitstream_md
    ...
    def bind(self, context, deployable):
        LOG.info(
            'Start bind jobs for ARQ(%s) with deployable(%s)',
            self.arq.uuid,
            deployable.uuid,
        )
        bs_id = self._get_bitstream_id()
    ...
    def _update_placement(self, context, function_id, driver_name):
        ...
        if not function_id:
            LOG.info(
                "Not get function id for resources provider %s.",
        ...
        try:
            placement.delete_traits_with_prefixes(
                context, self.arq.device_rp_uuid, [constants.FPGA_FUNCTION_ID]
            )
        except Exception as e:
            LOG.error(
                "Failed to delete traits(%s) from resources provider %s."
                "Reason: %s",
        ...
        trait_names = [
            "_".join((constants.FPGA_FUNCTION_ID, vendor, function_id))
        ]
        try:
            placement.add_traits_to_rp(self.arq.device_rp_uuid, trait_names)
        except Exception as e:
            LOG.error(
                "Failed to add traits(%s) to resources provider %s.Reason: %s",
                trait_names,
                self.arq.device_rp_uuid,
                e.message,
            )
            ...
            raise
        LOG.info(
            "Add traits(%s) to resources provider %s.",
            trait_names,
            self.arq.device_rp_uuid,
        )
    ...
    def _do_programming(self, context, deployable, bitstream_id):
        ...
        if count != 1:
            self.update_check_state(context, constants.ARQ_BIND_FAILED)
            raise exception.ExpectedOneObject(
                obj='controlpath_id', count=count
            )
        ...
        LOG.info(
            'Starting programming for host: (%s) deployable (%s) '
            'bitstream_id (%s)',
            hostname,
            deployable.uuid,
            bitstream_id,
        ...
        try:
            agent = AgentAPI()
            agent.fpga_program(
                context, hostname, controlpath_id, bitstream_id, driver_name
            )
        except Exception as e:
            self.update_check_state(context, constants.ARQ_BIND_FAILED)
            LOG.error(
                'Failed programming for host: (%s) deployable (%s). Error: %s',
                hostname,
                deployable.uuid,
                e.message,
            )
            raise
        LOG.info(
            'Finished programming for host: (%s) deployable (%s)',
            hostname,
            deployable.uuid,
        )
